...
def add_dataset_parser(parser: FlexibleArgumentParser):
    ...
    parser.add_argument(
        ...
        choices=[
            ...
            "custom", "prefix_repetition", "spec_bench", "synthetic_shared"
        ],
...
def get_samples(args, tokenizer) -> list[SampleRequest]:
    ...
    if args.dataset_name == "custom":
        ...
        dataset_mapping = {
            ...
            "synthetic_shared": lambda: SyntheticSharedDataset(
                random_seed=args.seed, dataset_path=args.dataset_path
            ).sample(
                tokenizer=tokenizer,
                num_requests=args.num_prompts,
...
# -----------------------------------------------------------------------------
# Synthetic Shared Dataset Implementation
# -----------------------------------------------------------------------------


class SyntheticSharedDataset(BenchmarkDataset):
    """
    Implements the Synthetic Shared dataset. Loads data from a JSONL file
    generated by aibrix-benchmarks/generator/dataset_generator/synthetic_prefix_sharing_dataset.py.
    The JSONL file contains prompts with prefix sharing information.
    Example format:
    {"prompt": "...", "token_count": 3985, "prefix_group": 0, "config_id": 0}
    """
    ...
    def load_data(self) -> None:
        ...
        # Load the JSONL file using standard json library (no pandas dependency)
        with open(self.dataset_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Check if the JSONL file has a 'prompt' field
                    if "prompt" not in data:
                        raise ValueError("JSONL file must contain a 'prompt' field.")
                    self.data.append(data)
                except json.JSONDecodeError as e:
                    logger.warning(f"Failed to decode JSON from line: {line[:100]}... Error: {e}")
                    continue
    ...
    def sample(
        ...
        **kwargs,
        ...
        for i, item in enumerate(self.data):
            ...
            # Use token_count from JSONL if available, otherwise calculate it
            if "token_count" in item and item["token_count"] is not None:
                prompt_len = item["token_count"]
            else:
                prompt_len = len(tokenizer(prompt).input_ids)
